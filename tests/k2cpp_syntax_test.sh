#!/bin/sh
# k2cpp syntax test — verifies the Kir-based .kry->C++ pipeline output.
set -eu
...
Valid :: (viewport: Rectangle) #ui {
    ...
    Screen root: {
        ...
        nav_items: [1] NavigationBarItem = {{1,"Home",(Texture2D){0},ICON_NONE,1,0}}
...
"$k2cpp" --root "$work" -o "$work/out" "$work/src/valid.kry" "$work/src/hierarchy.kry"
sh "$root/tests/check_clean_generated_output.sh" "$work/out"

c="$work/out/src/valid.cpp"
h="$work/out/src/valid.hpp"
hc="$work/out/src/hierarchy.cpp"
...
grep -Fq '"Hello"' "$hc"
grep -Fq 'DisabledScope(2 >= 1);' "$hc"
grep -Fq 'DisabledEndScope();' "$hc"
grep -Fq 'ScrollScope(viewport, 480, NULL)' "$hc"
grep -Fq 'ScrollScope(content, 0, NULL)' "$hc"
grep -Fq 'ScrollEndScope();' "$hc"
...
if "$k2cpp" --root "$work" -o "$work/out" "$work/src/assert_fail.kry" 2>"$work/assert_fail.err"; then
