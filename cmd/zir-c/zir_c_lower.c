...
/* Lower `.[e0, e1, ...]` file-scope array initializers. Record elements keep
 * designated C99 form via the record lowering; scalars pass through the body
 * rewriter so enum constants and named values resolve. */
static void
    ...
    if(init[0] != '.' || init[1] != '[' ||
       ...
       record->is_enum || record->is_procedure_type) {
        if(init[0] == '.' && init[1] == '[') {
            ...
            inner[length] = '\0';
            if(!rewrite_body2(m, NULL, 0, inner, rewritten,
                              sizeof(rewritten)))
                rewritten[0] = '\0';
    ...
    copy_text(out, size, "{");
    {
        ...
        while(*cursor && *cursor != ']') {
            ...
            entry[length] = '\0';
            {
                ...
                if(lowered[0] != '{') {
                    char rewritten[LOWER_TEXT_MAX];
                    if(!rewrite_body2(m, NULL, 0, entry, rewritten,
                                      sizeof(rewritten)))
                        rewritten[0] = '\0';
