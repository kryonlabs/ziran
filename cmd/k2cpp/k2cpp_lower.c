/*
 * k2cpp_lower.c - Kir → C++ backend. Lowers a KirProgram to .cpp/.hpp
 * source files that call the C Kryon runtime. Emitted declarations keep
 * C linkage (extern "C") so k2cpp output interops with k2c modules and the
...
static size_t
resolve_aliased_fn(const KirModule *m, const K2cppModuleSyms *restab,
...
void
k2cpp_function_c_name(const KirModule *m, const KirFunction *fn,
                    char *dst, size_t dst_size)
{
...
static int k2cpp_in_array_init;

static void
emit_call_wrap(FILE *c, const KirModule *m, const K2cppModuleSyms *restab,
    ...
    for(j = 0; j < fn->stmt_count; j++) {
        ...
        case KIR_STMT_DECL: {
            ...
            if(n >= 2) {
                ...
                for(int k = 0; k < n; k++) {
                    emit_indent(c, indent);
                    fprintf(c, "auto __kryon_assign_%d_%d = %s;\n",
...
static void
lower_module(const KirModule *m, const K2cppModuleSyms *restab, int restab_count, const char *out_dir)
{
    char stem[512];
    char guard[600];
    char hpath[1024];
    ...
    fprintf(h, "#ifndef %s\n#define %s\n\n#include <stdint.h>\n#include <stdbool.h>\n", guard, guard);
    fputs("#include \"kry_bounds.h\"\n", h);
    if(KirModuleUsesSlices(m))
        fputs("#include \"kry_slice.h\"\n", h);
    KirEmitStringType(h);
    for(i = 0; i < m->import_count; i++) {
        const KirImport *imp = &m->imports[i];
    ...
    for(i = 0; i < m->type_count; i++) {
        ...
        KirEmitSlotType(h, slot, KIR_CPP, NULL, NULL);
    ...
    for(i = 0; i < m->function_count; i++) {
        ...
        strip_alias_type(m, KirArrayElementType(fn->return_type, NULL, 0, NULL) ? "void" : fn->return_type,
                         cret, sizeof(cret));
    ...
    if(c == NULL)
        return;
    fprintf(c, "/* Generated by k2cpp from %s. */\n", m->source_path);
    ...
    for(i = 0; i < m->state_count; i++) {
        ...
        if(!KirScalarLiteral(f->type,f->init[0]?f->init:"0",KIR_CPP,f->span,init,sizeof(init)))
    ...
    for(i = 0; i < m->function_count; i++) {
        ...
        emit_guard_open(c, fn->guard);
        BodySymbols symbols = {m, restab, restab_count};
        KirEmitSlotWrappers(c, m, fn, KIR_CPP, resolve_body_symbol, &symbols);
        if(fn->is_public)
            fprintf(c, "%s\n%s(%s)\n{\n", cret[0] ? cret : "void",
                    cname, cargs);
        else
...
void
k2cpp_lower(const KirProgram *program, const char *root, const char *out_dir, const K2cppModuleSyms *restab, int restab_count)
{
    int i;
...
void
k2cpp_build_syms(const KirProgram *program, K2cppModuleSyms *out)
{
    int i, j;
