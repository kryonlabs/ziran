...
/* Post-lowering pass that makes k2c output safe for the native Plan 9
 ...
 * resolving __auto_type initializers. Call before k2c_lower. */
void k2c_plan9_add_include_dir(const char *dir);

/* Enable --plan9 output post-processing (off by default; the default
 * output is unchanged). */
void k2c_plan9_set_enabled(int enabled);
int k2c_plan9_enabled(void);
...
char *k2c_plan9_rewrite(const char *text);
...
int k2c_plan9_rewrite_file(const char *path, int is_project);

/* Number of __auto_type declarations left unresolved by the last
 * k2c_plan9_rewrite call (reported once per run by main). */
int k2c_plan9_unresolved(void);
 ...
 * prototypes. */
char *k2c_plan9_rewrite_project(const char *text);
