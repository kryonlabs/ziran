...
static void
emit_c_string_literal(FILE *out, const char *src)
{
    fputc('"', out);
    for(const unsigned char *p = (const unsigned char *)(src != NULL ? src : "");
        *p != '\0'; p++) {
        if(*p == '"' || *p == '\\')
            fprintf(out, "\\%c", *p);
        else if(*p == '\n')
...
static void
emit_style_registration(FILE *out, KirProgram *const *progs, int prog_count,
                        const char *root)
{
    int emitted = 0;
    ...
    for(int i = 0; i < prog_count; i++) {
        ...
        for(int m = 0; m < prog->module_count; m++) {
            ...
            for(int s = 0; s < mod->style_import_count; s++) {
                const KirStyleImport *style = &mod->style_imports[s];
                char *source;

                if(!emitted) {
                    fprintf(out, "static void\nkryon_project_register_styles(void)\n{\n");
                    emitted = 1;
                }
                if(style->kind == KIR_STYLE_IMPORT_BUILTIN &&
                   KirStyleImportIsBuiltIn(style->target)) {
                    fprintf(out, "    EnsureBuiltInStylePacks();\n");
                    fprintf(out, "    SetActiveStylePack(");
                    emit_c_string_literal(out, style->target);
                    fprintf(out, ");\n");
                    continue;
                }
                source = KirReadStyleImportSource(mod, root, style);
                if(source == NULL) {
                    fprintf(stderr, "%s:%d: unable to resolve #style %s\n",
                            style->span.path, style->span.line, style->target);
                    continue;
                }
                fprintf(out, "    if(RegisterStylePackSource(");
                emit_c_string_literal(out, source);
                fprintf(out, ", ");
                emit_c_string_literal(out,
                    style->alias[0] != '\0' ? style->alias : style->target);
                fprintf(out, ", \"\"))\n");
                fprintf(out, "        SetActiveStylePack(");
                emit_c_string_literal(out,
                    style->alias[0] != '\0' ? style->alias : style->target);
...
void
k2c_write_project(KirProgram *const *progs, int prog_count,
                  const char *root, const char *out_dir, int no_main)
{
    K2cRoute routes[K2C_ROUTE_MAX];
    ...
    if(out == NULL)
        return;
    fprintf(out, "/* Generated by k2c from project Kry files. */\n");
    ...
    for(int i = 0; i < count; i++) {
        ...
        k2c_function_c_name(routes[i].m, routes[i].fn, cname, sizeof(cname));
    ...
    fprintf(out, "    kryon_project_register_styles();\n");
