...
append_initializer(char *dst, size_t size, size_t *used, const char *format, ...)
{
    ...
    if(written < 0 || (size_t)written >= size - *used) {
        fprintf(stderr, "zir_cpp: named initializer exceeds output limit\n");
        exit(1);
    }
    *used += (size_t)written;
}
                          ...
                          size_t dst_size, const char *shadow)
{
    ...
    for(int i = 0; i < count; i++) {
        ...
        if(*field != '.' || equals == NULL) {
            fprintf(stderr, "zir_cpp: expected named initializer field\n");
            exit(1);
        }
        *equals = '\0';
...
static int zir_cpp_in_array_init;

static void
emit_call_wrap(FILE *c, const ZirModule *m, const ZirCppModuleSyms *restab,
...
lower_module(const ZirModule *m, const ZirCppModuleSyms *restab, int restab_count, const char *out_dir)
{
    ...
    if(c == NULL)
        return;
    fprintf(c, "/* Generated by zir_cpp from %s. */\n", m->source_path);
