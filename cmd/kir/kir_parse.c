                       ...
                       char *args, size_t args_size)
{
    static const char *const widgets[] = {
        ...
        "BlockQuote", "Bold", "Br", "Cite", "Code", "CodeBlock",
        ...
        "ImageMap", "Kbd", "Keyboard", "Label", "Legend", "LineBreak", "List", "ListItem", "Main",
        "Mark", "Meta", "Meter", "Nav", "Navigation", "NoScript", "Noscript", "EmbeddedObject", "OrderedList",
...
is_web_native_block_widget(const char *name)
{
    static const char *const widgets[] = {
        ...
        "Legend", "LineBreak",
        "List", "ListItem", "Main", "Mark", "Meta", "Nav", "Navigation", "NoScript", "Noscript",
        "EmbeddedObject", "OrderedList", "OptionGroup", "OptGroup", "Option", "Output", "Param", "Pre", "Quote",
        "Rp", "Rt", "Ruby", "RubyParenthesis", "RubyText",
        "Samp", "Sample", "Script", "Search", "Select", "Slot", "Small", "Source", "Strong", "StyleElement", "Sub", "Subscript",
        "Summary", "Sup", "Superscript", "Table", "TableBody",
        "TableCaption", "TableColumnGroup", "TableFoot", "TableHead",
        "TableRow", "Tbody", "Template", "Tfoot", "Thead", "Time", "Title", "Tr", "Track",
                                         ...
                                         KirSourceSpan span)
{
    ...
    if(kind == KIR_STMT_RETURN && starts_word(expr, "return")) {
        ...
        kir_trim_in_place(expr);
    } else if(kind == KIR_STMT_IF || kind == KIR_STMT_WHILE ||
              kind == KIR_STMT_SWITCH) {
        kir_strip_block_brace(expr);
        if(kind == KIR_STMT_WHILE && starts_word(expr, "while")) {
            memmove(expr, expr + 5, strlen(expr + 5) + 1);
            kir_trim_in_place(expr);
        } else if(kind == KIR_STMT_SWITCH && starts_word(expr, "switch")) {
            memmove(expr, expr + 6, strlen(expr + 6) + 1);
            kir_trim_in_place(expr);
        } else if(starts_word(expr, "else if")) {
