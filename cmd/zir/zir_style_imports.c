...
int
ZirStyleImportIsBuiltIn(const char *target)
{
...
char *
ZirReadStyleImportSource(const ZirModule *module, const char *root,
                         const ZirStyleImport *style)
{
    char relative[ZIR_PATH_MAX * 2];
    char dotted_relative[ZIR_PATH_MAX * 2];
    char path[ZIR_PATH_MAX * 4];
    char module_dir[ZIR_PATH_MAX];
    char *text;

    if(style->kind == ZIR_STYLE_IMPORT_BUILTIN) {
        if(ZirStyleImportIsBuiltIn(style->target))
            return NULL;
